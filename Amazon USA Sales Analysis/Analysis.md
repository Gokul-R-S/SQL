...
FROM CROSSTAB(
  $$ SELECT
    ...
    ROUND(SUM(o.quantity * o.price_per_unit)::NUMERIC, 2) AS revenue
  FROM dim_products p
  JOIN fact_orders o USING (product_id)
  WHERE o.order_status <> 'Cancelled'
    AND o.order_date >= '2023-01-01'
    AND o.order_date < '2024-04-01'
  GROUP BY p.category_name, TO_CHAR(o.order_date, 'YYYY-"Q"Q')
  ORDER BY p.category_name, quarter $$,
...
SELECT 
    category_name,
    ROUND((SUM(quantity * price_per_unit) / SUM(SUM(quantity * price_per_unit)) OVER () * 100)::NUMERIC, 2) AS contribution_pct
FROM fact_orders
...
WITH cte AS (
    SELECT
        c.state,
        p.category_name,
        ROUND(SUM(o.quantity * o.price_per_unit)::NUMERIC,2) AS total_sale,
        RANK() OVER (PARTITION BY c.state ORDER BY SUM(o.quantity * o.price_per_unit) ASC) AS rnk
    FROM fact_orders o
    JOIN dim_customers c USING (customer_id)
    JOIN dim_products p USING (product_id)
    GROUP BY c.state, p.category_name
)
SELECT 
    state, category_name, total_sale
FROM cte
WHERE rnk = 1;
```
...
WITH product_orders AS (
    SELECT
        ...
        o.seller_id,
        s.seller_name,
        o.shipping_providers,
        o.delivery_status,
        o.price_per_unit * o.quantity AS order_value,
        p.cogs * o.quantity AS order_cost,
        (o.delivery_status = 'Returned')::int AS is_returned
    FROM dim_products p
    JOIN fact_orders o USING (product_id)
...
product_metrics AS (
    SELECT
        ...
        ROUND((SUM(is_returned)::DECIMAL / NULLIF(COUNT(DISTINCT order_id), 0)) * 100, 2) AS return_rate_pct
    FROM product_orders
...
SELECT
    ...
    ROUND(pm.revenue_lost_to_returns::NUMERIC, 2) AS revenue_lost_to_returns,
    ROUND(pm.cost_of_returns::NUMERIC, 2) AS cost_of_returns,
    ROUND((pm.revenue_lost_to_returns + pm.cost_of_returns)::NUMERIC, 2) AS total_return_impact,
...
LEFT JOIN seller_return_analysis sra
    ON pm.product_id = sra.product_id AND sra.seller_rank = 1
LEFT JOIN shipping_return_analysis sha
    ON pm.product_id = sha.product_id AND sha.provider_rank = 1
...
WITH yearly_revenue AS (
  ...
  GROUP BY p.product_id, p.product_name, p.category_name, yr
),
ranked_products AS (
  SELECT
    curr.product_id,
    ...
    ROUND(((curr.revenue - prev.revenue)/ NULLIF(prev.revenue, 0) * 100)::NUMERIC , 2) AS revenue_decrease_ratio,
    RANK() OVER (ORDER BY (curr.revenue - prev.revenue) / NULLIF(prev.revenue, 0)) AS rnk
...
SELECT  
    ...
    ROUND(COALESCE(SUM(o.quantity * o.price_per_unit), 0)::NUMERIC,2) AS total_sales
FROM inactive_sellers ins
LEFT JOIN fact_orders o ON o.seller_id = ins.seller_id
...
WITH recent_sales AS (
    SELECT
        ...
        p.price,
        p.cogs,
        ROUND(((p.price - p.cogs) / NULLIF(p.price, 0) * 100)::NUMERIC, 2) AS profit_margin_pct,
        COUNT(DISTINCT o.order_id) AS num_orders,
        SUM(o.quantity) AS units_sold_6months,
        SUM(o.quantity * o.price_per_unit) AS revenue_6months,
...
SELECT
    ...
    profit_margin_pct,
    units_sold_6months,
    ROUND(revenue_6months::NUMERIC, 2) AS revenue_6months,
    current_stock,
...
WITH seller_performance AS (
    SELECT
        ...
        ROUND(AVG(o.quantity * o.price_per_unit)::NUMERIC, 2) AS avg_order_value
    FROM dim_sellers s
    JOIN fact_orders o USING (seller_id)
    WHERE o.order_status <> 'Cancelled'
    GROUP BY s.seller_id, s.seller_name, s.origin
),
platform_avg AS (
    SELECT
        ROUND(AVG(quantity * price_per_unit)::NUMERIC, 2) AS platform_avg_order_value
    FROM fact_orders
...
seller_category_performance AS (
    ...
    JOIN dim_products p USING (product_id)
    WHERE o.order_status <> 'Cancelled'
    GROUP BY o.seller_id, p.category_name
),
high_value_categories AS (
    -- Categories with avg order value > 500
    SELECT
        p.category_name,
        ROUND(AVG(o.quantity * o.price_per_unit)::NUMERIC, 2) AS category_avg_order_value
    FROM dim_products p
    JOIN fact_orders o USING (product_id)
    WHERE o.order_status <> 'Cancelled'
    GROUP BY p.category_name
    HAVING AVG(o.quantity * o.price_per_unit) > 500
),
seller_sold_categories AS (
    -- Categories each seller has already sold in
    SELECT DISTINCT
        o.seller_id,
        p.category_name
    FROM fact_orders o
...
seller_recommendations AS (
    -- Cross join sellers with high value categories, exclude ones they already sell in
    SELECT
        sp.seller_id,
        STRING_AGG(hvc.category_name, ', ' ORDER BY hvc.category_avg_order_value DESC) AS recommended_high_value_categories
    FROM seller_performance sp
    CROSS JOIN high_value_categories hvc
    WHERE NOT EXISTS (
        SELECT 1
        FROM seller_sold_categories ssc
        WHERE ssc.seller_id = sp.seller_id
          AND ssc.category_name = hvc.category_name
    )
...
SELECT
    ...
    sp.origin,
    sp.total_orders,
    ROUND(sp.total_revenue::NUMERIC, 2) AS current_revenue,
    sp.avg_order_value AS current_avg_order_value,
    pa.platform_avg_order_value,
    ROUND((sp.avg_order_value - pa.platform_avg_order_value)::NUMERIC, 2) AS performance_gap,
    ROUND(((pa.platform_avg_order_value - sp.avg_order_value) * sp.total_orders)::NUMERIC, 2) AS revenue_opportunity,
    scp.category_name AS current_best_category,
    ROUND(scp.category_revenue::NUMERIC, 2) AS best_category_revenue,
    COALESCE(sr.recommended_high_value_categories, 'No New High Value Categories Available') AS recommended_high_value_categories
FROM seller_performance sp
CROSS JOIN platform_avg pa
LEFT JOIN seller_category_performance scp
    ON sp.seller_id = scp.seller_id
    AND scp.category_rank = 1
LEFT JOIN seller_recommendations sr
    ON sp.seller_id = sr.seller_id
